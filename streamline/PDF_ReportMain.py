...
def main(argv):
    ...
    if not options.do_check: #Run job submission
        ...
        if eval(options.run_parallel):
            ...
            submitSlurmClusterJob(experiment_path,options.training,options.rep_data_path,options.dataset_for_rep,options.reserved_memory,options.maximum_memory,options.queue)
        else:
...
def submitSlurmClusterJob(experiment_path,training,rep_data_path,dataset_for_rep,reserved_memory,maximum_memory,queue):
    """ Runs PDF_ReportJob.py once. Runs on a linux-based computing cluster that uses an IBM Spectrum LSF for job scheduling."""
    job_ref = str(time.time())
    job_name = experiment_path + '/jobs/PDF_' + job_ref + '_run.sh'
    sh_file = open(job_name,'w')
    ...
    this_file_path = os.path.dirname(os.path.realpath(__file__))
    sh_file.write('srun python3 ' + this_file_path + '/PDF_ReportJob.py ' + experiment_path+' '+training+' '+rep_data_path+' '+dataset_for_rep + '\n')
    sh_file.close()
